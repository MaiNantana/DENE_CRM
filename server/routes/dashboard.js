...
router.get('/', aw(async (req, res) => {
  ...
  const [[{ totalUsers }]] = await db.query('SELECT COUNT(*) AS totalUsers FROM users WHERE company_id=? AND is_member=1', [companyId]);
  ...
  const [[{ totalPoints }]] = await db.query('SELECT COALESCE(SUM(points),0) AS totalPoints FROM users WHERE company_id=? AND is_member=1', [companyId]);
  ...
  const [tierBreakdown] = await db.query('SELECT tier, COUNT(*) AS count FROM users WHERE company_id=? AND is_member=1 GROUP BY tier', [companyId]);
